    ...
    def screen_filtering(self):
        ...
        def rs_button_event():
            ...
            def resize_left(event):
                old_left = rs_win.winfo_x()
                rs_win.geometry("+" + str(event.x_root) + "+" + str(rs_win.winfo_y()))
                rs_win.geometry(str(rs_win.current_width + old_left - event.x_root) + "x" + str(rs_win.current_height))
                left_side.place(x=0, y=0)
                right_side.place(x=rs_win.current_width - 10, y=0)

            def resize_right(event):
                rs_win.geometry(str(event.x_root) + "x" + str(rs_win.current_height))
                right_side.place(x=rs_win.current_width - 10, y=0)

            def resize_top(event):
                old_top = rs_win.winfo_y()
                rs_win.geometry("+" + str(rs_win.winfo_x()) + "+" + str(event.y_root))
                rs_win.geometry(str(rs_win.current_width) + "x" + str(rs_win.current_height + old_top - event.y_root))
                top_side.place(x=0, y=0)
                bottom_side.place(x=0, y=rs_win.current_height - 10)

            def resize_bottom(event):
                rs_win.geometry(str(rs_win.current_width) + "x" + str(event.y_root))
                bottom_side.place(x=0, y=rs_win.current_height - 10)
